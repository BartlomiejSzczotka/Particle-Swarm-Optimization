# -*- coding: utf-8 -*-
"""
Created on Sun Feb 10 15:02:02 2019
...
def rastriginfunction(x,y):#for n = 2
...
ax.contourf(ux,uy,Z) #plotting the contour map (filled)
...
def get_gbest(position_array):
    ...
    for pos in position_array:
        if(rastriginfunction(pos[0],pos[1])<rastriginfunction(x_best,y_best)):
...
for i in range(number_generations):
    for j in range(total_population):
        ...
        if(rastriginfunction(position[j][0],position[j][1])<rastriginfunction(best_position[j][0],best_position[j][1])):
...
print("\nValue of these coordinates : "+ str(rastriginfunction(best[0][0],best[0][1])) + "\n")
